<div>
    <button @click.prevent="$wire.ModalEditAppointment = true" class="btn btn-warning">
        <i class="fa-thin fa-calendar"></i> {{ $appointment->appointment_type == 'writing' ? 'Répondre' : 'Modifier le RDV' }}
    </button>

    @if($appointment->appointment_type != 'writing')
        <x-ui.dialog wire:model="ModalEditAppointment" title="Modifier le rendez-vous" subtitle="Gestion du rendez-vous programmé le {{ $initialeDate }}" class="text-left">
    ...
    @else
        <x-ui.dialog wire:model="ModalEditAppointment" title="Répondre à la question écrite de {{ $name }}" class="text-left">
            {{-- Loader --}}
            <div class="relative" wire:loading>
                <x-ui.loader :loadingText="false" :overlay="false" />
            </div>
